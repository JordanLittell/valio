...
/** GET JSON from a peer. Never throws: transport and HTTP failures become { ok: false }. */
export async function get<T>(baseUrl: string, path: string): Promise<PeerResult<T>> {
  try {
    const res = await fetch(baseUrl + path, { signal: AbortSignal.timeout(PEER_TIMEOUT_MS) });
